    ...
    async fn connect_to_remote_peer(config: &Config) -> Result<TcpStream> {
        ...
        TcpStream::connect((config.remote_ip, bgp_port))
            ...
            .context(format!(
                ...
                config.remote_ip, bgq_port
            ))
    }

    async fn wait_connection_from_remote_peer(config: &Config) -> Result<TcpStream> {
        let bgq_port = 179;
        let listener = TcpListener::bind((config.local_ip, bgq_port))
            .await
            .context(format!(
        ...
        Ok(listener
            ...
            .context(format!(
                "{0}:{1}にてリモートからの \
                TCP Connectionの要求を完遂することができませんでした。 \
                リモートからTCP Connectionの要求が来ていない可能性が高いです。",
                config.local_ip, bgq_port
            ))?
            .0)
    }
}
